...
def run():
    with sync_playwright() as p:
        ...
        # --- Step 2: Select Category (OPPORTUNITY CARD) ---
        print(">> Selecting 'Opportunity Card' category...")
        try:
            # XPath Logic: 
            # 1. Find element containing unique text "Opportunity Card"
            # 2. Click the 'Continue' link immediately following it.
            page.locator("xpath=//*[contains(text(), 'Opportunity Card')]//following::a[contains(text(), 'Continue')]").first.click()
        ...
        try:
            page.get_by_role("link", name="Continue").first.click()
             ...
             # Fallback if specific link role fails
        ...
        for i in range(max_months_to_check):
            # Check for any link that indicates an appointment slot (usually "Appointments are available" or specific dates)
        ...
        if not found_date:
            ...
            # We keep it open so you can see the result
            time.sleep(60)
            browser.close()
            return
        ...
        # --- Step 7: Fill Information (ROBUST FIX) ---
        print(">> Filling personal details...")
        page.wait_for_load_state("domcontentloaded")

        try:
            # 1. Last Name
            page.fill("input[name='lastname']", "NEETHIRASA")
            
            # 2. First Name
            page.fill("input[name='firstname']", "NILAVINTHAN")
            
            # 3. Email
            page.fill("input[name='email']", "nilavinthanoffer15@outlook.com")
            
            # 4. Repeat Email (Robust)
            print(">> Filling Repeat Email...")
            page.locator("xpath=//*[contains(text(), 'Repeat email')]//following::input[1]").fill("nilavinthanoffer15@outlook.com")

            # 5. Passport Number
            print(">> Filling Passport Number...")
            page.locator("xpath=//*[contains(text(), 'Passport Number')]//following::input[1]").fill("P0225105")

            # 6. Telephone Number (Strategy A & B)
            print(">> Filling Telephone...")
            if page.locator("input[name='telephonenumber']").count() > 0:
                 page.fill("input[name='telephonenumber']", "0763516060")
            else:
                 # Fallback: search for text "Telephone" loosely
                 page.locator("xpath=//*[contains(text(), 'Telephone')]//following::input[1]").first.fill("0763516060")
        ...
        # --- Keep Browser Open ---
        print(">> Done. Browser will remain open for 5 minutes.")
        time.sleep(300)
        browser.close()
