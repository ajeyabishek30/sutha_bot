...
def run():
    with sync_playwright() as p:
        ...
        page.goto(
            "https://service2.diplo.de/rktermin/extern/choose_categoryList.do?locationCode=colo&realmId=1419&request_locale=en"
        )
        ...
        try:
            ...
            page.locator(
                "xpath=//*[contains(text(), 'Students') and not(contains(text(), 'except'))]//following::a[contains(text(), 'Continue')]"
            ).first.click()
        ...
        for attempt in range(2):
            ...
            print(
                f">> ATTEMPT {attempt + 1}: WAITING {wait_time} SECONDS FOR MANUAL CAPTCHA."
            )
            ...
            try:
                ...
                page.wait_for_selector(
                    "text=Please select a date", timeout=5000
                )
        ...
        for i in range(max_months):
            ...
            if next_arrow.count() > 0:
                print(
                    f">> No slots in month {i+1}. Checking next month..."
                )
                next_arrow.click()
                page.wait_for_load_state("networkidle")
                time.sleep(1)
        ...
        # --- Step 7: Fill Information ---
        print(">> Filling personal details...")
        page.wait_for_load_state("domcontentloaded")

        try:
            # 1. Last Name (Surname)
            page.fill("input[name='lastname']", "GEORGELENIN")

            # 2. First Name (Given Name)
            page.fill("input[name='firstname']", "LIJOHNS")

            # 3. Email
            page.fill("input[name='email']", "georgelijohns871@gmail.com")

            # 4. Repeat Email
            page.locator(
                "xpath=//*[contains(text(), 'Repeat email')]//following::input[1]"
            ).fill("georgelijohns871@gmail.com")

            # 5. Passport Number
            page.locator(
                "xpath=//*[contains(text(), 'Passport Number')]//following::input[1]"
            ).fill("P0578923")
            ...
            if page.locator("input[name='telephonenumber']").count() > 0:
                ...
                page.locator(
                    "xpath=//*[contains(text(), 'Telephone')]//following::input[1]"
                ).first.fill(phone)
        ...
        # --- Keep Browser Open ---
        print(
            ">> Done. Browser will remain open for 2 minutes for you to review."
        )
        time.sleep(120)
        browser.close()
