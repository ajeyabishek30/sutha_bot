...
def run():
    with sync_playwright() as p:
        ...
        print(">> Filling personal details for RUKSHIKA MANISEGARAN...")
        page.wait_for_load_state("domcontentloaded")

        try:
            # 1. Last Name
            page.fill("input[name='lastname']", "MANISEGARAN")
            
            # 2. First Name
            page.fill("input[name='firstname']", "RUKSHIKA")
            
            # 3. Email
            page.fill("input[name='email']", "manisegaranrukshika10@gmail.com")
            
            # 4. Repeat Email
            page.locator("xpath=//*[contains(text(), 'Repeat email')]//following::input[1]").fill("manisegaranrukshika10@gmail.com")

            # 5. Passport Number
            page.locator("xpath=//*[contains(text(), 'Passport Number')]//following::input[1]").fill("N11455081")
